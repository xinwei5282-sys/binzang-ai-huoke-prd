...
> **版本**：v3.0 ｜ **状态**：当前开发真源 ｜ **日期**：2026-08-12
...
产品采用五个一级模块：`首页`、`品牌与经营`、`营销物料`、`AI 获客`、`企业大脑`，以及独立的`设置`。企业大脑维护可被 AI 使用的事实、VI 和外部证据；内容模块消费已确认上下文；发布任务必须绑定具体账号和发布方式；所有高风险事实、对外承诺、正式 VI、外部情报采用和 RPA 执行均保留人工确认。
...
| 企业大脑 | 企业 VI | 偏好、方向、Logo、场景图、启用、下载与历史版本 |
...
**流程**：企业偏好 → 生成三套方向及 Logo → 查看具体方向展示 → 选择方向 → 推荐应用场景 → 场景图片独立生成与审核 → 完整 VI 草案 → 人工确认启用 → 下载全套 VI。
**状态机**：
...
**图片任务状态**：`queued → generating → review → adopted | rejected`；失败为 `failed → queued`。
**启用条件**：基础视觉任务全部 `adopted`；场景任务全部完成审核且至少一张 `adopted`。
...
**独立编辑页**：列表点击“预览 / 编辑”新开页面，原列表保留。详情页左侧为缩略图，中间为当前页画布，右侧只保留 AI 追问。标题和正文在画布内直接编辑并自动保存；不使用右侧结构化表单和“编辑 / 预览 / AI”模式切换。

**AI 追问**：每次只对当前页生成候选版。候选版未采用前不覆盖原页；采用后只更新当前页并递增整套 PPT 版本号，撤回或失败不影响原页和已有下载版本。

**交互边界**：保留页面切换、上一页/下一页、全屏播放和下载；本期不支持自由拖拽、图层编辑、任意坐标排版或矢量文件编辑。
...
PPT 不使用通用内容审核状态 `pending_review | approved`。用户只审核大纲；确认后进入后台异步生成，成品为 `ready` 时可直接下载或进入独立画布编辑页。画布中的标题和正文支持直接编辑，右侧只保留当前页 AI 追问；AI 结果先进入 `pending_candidate`，采用后才覆盖当前页并递增 `deck_version`。
...
所有对外发布、正式知识采用、VI 启用、价格与承诺变更都必须等待人工确认。
...
只生成候选方向，status=directions_ready，不得自动启用。
```

### P-021 企业 VI 单图任务

```text
按一个 scene_id 生成一张 VI 应用图。严格使用 brand_seed 的 Logo、颜色、字体层级与图片风格。
...
| AC-003 | 已有 active VI v1 | 生成 v2 草案但未启用 | 内容生成仍引用 v1 |
| AC-004 | VI 场景图一张失败 | 点击单张重试 | 仅该图重试，其他图状态不变 |
...
| AC-018 | 打开 PPT 编辑页 | 检查右侧面板 | 只显示 AI 追问及候选版操作，无结构化编辑表单和模式切换 |
...
| DEV-005 | 企业 VI 方向、单图任务、启用、版本和下载任务 | DEV-001/004 | VI-001、AC-003/004 |
...
| DEV-008 | PPT 大纲审核、异步生成、独立编辑页与下载 | DEV-007 | MAT-PPT-001、AC-013–018 |
