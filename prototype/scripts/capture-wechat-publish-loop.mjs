...
const failed = !before.pageVisible || !before.panelVisible || !before.focusedActive || before.accountStatus !== '已授权' || !before.autoPublish || before.publishAction !== '审核并发布' || before.coreLabels.length !== 6 || before.accountCount !== 2 || before.rowAccount !== '示例企业服务' || !before.metricsText.includes('发布后回收') || before.horizontalOverflow || before.outside.length || afterPublish.publishStatus !== '已发布' || afterPublish.rowAction !== '刷新数据' || JSON.stringify(afterPublish.metrics) !== JSON.stringify({reads:1286,shares:46,likes:83,follows:17}) || !afterPublish.published || afterPublish.account !== '示例企业服务' || !afterPublish.toast.includes('首次数据已回收') || JSON.stringify(afterManualSync.metrics) !== JSON.stringify({reads:1314,shares:48,likes:86,follows:18}) || !afterManualSync.toast.includes('本篇文章数据已刷新') || afterManualSync.horizontalOverflow || consoleErrors.length || uncaughtErrors.length;
if (failed) {
