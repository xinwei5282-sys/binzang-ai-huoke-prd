...
test('enterprise brain exposes five non-duplicated responsibility panels', () => {
  ...
  for (const phrase of ['企业信息完整度', '企业偏好与视觉规范', '系统生成内容立即成为正式知识', '竞对、政策与行业候选', '修改、发布、删除与学习候选']) {
...
test('Geyou stays a configurable large customer', () => {
  for (const phrase of ['示例客户模式', '行业配置包', '90 天试点模板']) assert.match(html, new RegExp(phrase));
