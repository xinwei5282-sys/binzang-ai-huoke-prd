...
test('consumer multimodal knowledge and agents keep explicit visibility and action boundaries', () => {
  for (const token of [
    '原件已保留', '已生成转写、描述与标签', '对外使用待版权确认',
    '消费者问答 Agent', '数字人讲解 Agent', '内容审核 Agent',
    '只审核，不直接发布', '富媒体输出需审核'
  ]) assert.match(html, new RegExp(token));
  assert.match(html, /function renderConsumerKnowledgeItems\(/);
  assert.match(html, /function renderConsumerAgents\(/);
});
